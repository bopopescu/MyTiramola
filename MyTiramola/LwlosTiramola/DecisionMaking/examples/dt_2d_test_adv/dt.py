...
def get_load(time):
    return 50.0 + 50.0 * math.sin(2 * math.pi * time / 1000.0)
...
CONFIGURATION_FILE = TIRAMOLA_DIR + "examples/dt_2d_test_adv/dt.json"
...
while time < max_steps:
    ...
    if time == max_steps - print_steps:
        model.root.split(TOTAL_LOAD, [10, 20, 30, 40, 50, 60, 70, 80, 90])
        model.value_iteration(0.01)

    if time % 100 == 1 and time > max_steps - print_steps:
        model.value_iteration(0.01)
