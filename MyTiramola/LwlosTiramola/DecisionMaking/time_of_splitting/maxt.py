...
load_period    = 250
epsilon        = 0.5
split_strategy = "half_chain"
split_crit     = MAX_POINT
cons_trans     = False
CONF_FILE      = TIRAMOLA_DIR + "time_of_splitting/maxt.json"
##############################################################

if len(sys.argv) > 1:
    split_strategy = sys.argv[1]
else:
    print "you must provide a split strategy"
    exit()

strategies = ["start", "start_chain", "half", "half_chain", "end", "always"]
if not split_strategy in strategies:
