...
cons_trans     = True
CONF_FILE      = TIRAMOLA_DIR + "OS/mdpdt.json"
##############################################################
...
for i in range(num_tests):
    ...
    scenario = OSScenario(load_period=load_period, min_vms=MIN_VMS, max_vms=MAX_VMS)
    ...
    model.set_prioritized_sweeping(0.1, 200)
    ...
    for time in range(training_steps + eval_steps):
        ...
        if PRINT and time > training_steps:
            #print meas['incoming_load'], scenario.get_current_capacity()
            print meas['incoming_load'], 3 + 3 * meas[NUMBER_OF_VMS]
...
print "Latest splits per parameter: " + str(splits)
print ""
